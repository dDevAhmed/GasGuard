...
impl SorobanParser {
    ...
    pub fn parse_contract(source: &str, file_path: &str) -> SorobanResult<SorobanContract> {
        ...
        let contract_name =
            Self::extract_contract_name(source).unwrap_or_else(|_| "UnknownContract".to_string());
    ...
    fn parse_contract_types(lines: &[&str]) -> SorobanResult<Vec<SorobanStruct>> {
        ...
        while i < lines.len() {
            if lines[i].trim().starts_with("#[contracttype]") {
                ...
                i += 1;
                while i < lines.len()
                    && !lines[i].trim().starts_with("struct")
                    && !lines[i].trim().starts_with("pub struct")
                {
                    i += 1;
                }
    ...
    fn parse_single_struct(
        ...
        if lines.is_empty()
            || (!lines[0].trim().starts_with("struct")
                && !lines[0].trim().starts_with("pub struct"))
        {
            return Ok(None);
        }
